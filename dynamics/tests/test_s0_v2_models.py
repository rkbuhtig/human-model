...
class V2ModelTests(unittest.TestCase):
    ...
    def test_report_and_certified_occurrence_have_different_weight(self):
        ...
        base = initialize_h(context, "t1").predictive_state
    ...
    def test_action_authorship_dimensions_do_not_collapse(self):
        state = h_full_state(make_prefix(["SELF_ACTION_CAUSALLY_ATTRIBUTED"])).predictive_state
    ...
    def test_full_and_incremental_state_match(self):
        ...
        self.assertEqual(predict_full("H", prefix, self.parameters), predict_h_state_only(state, prefix.context, self.parameters))

    def test_state_only_runner_forbids_history_and_reports_no_reread(self):
        prefix = make_prefix(["CURRENT_COMMITMENT_MISSED"])
        initial = initialize_h(prefix.context, prefix.key.trajectory_id)
    ...
    def test_incremental_state_rejects_duplicate_cross_trajectory_and_tamper(self):
        prefix = make_prefix(["CURRENT_COMMITMENT_MISSED"])
        initial = initialize_h(prefix.context, "t1")
    ...
    def test_multi_clock_fast_decay_and_slow_consolidation(self):
        ...
        state = initialize_h(context, "t1")
        after_breach = step_h(state, PublicDeltaV2(SCHEMA_VERSION, PredictionKey("TEST-INSTANCE-0000", "t1", 1, "p1"), context, receipt(1, "CURRENT_COMMITMENT_MISSED")))
        after_idle = step_h(after_breach, PublicDeltaV2(SCHEMA_VERSION, PredictionKey("TEST-INSTANCE-0000", "t1", 2, "p2"), context, receipt(2, "NO_NEW_INFORMATION", actor="environment", target="interaction")))
        ...
        after_repair = step_h(after_idle, PublicDeltaV2(SCHEMA_VERSION, PredictionKey("TEST-INSTANCE-0000", "t1", 3, "p3"), context, receipt(3, "COUNTERPART_OFFERS_COSTLY_REPAIR")))
    ...
    def test_full_prediction_has_no_directional_common_output(self):
        self.assertNotIn("directional_readout", run_full("B2", make_prefix(["CURRENT_COMMITMENT_MISSED"]).to_dict(), self.parameters))
...
class V2FairnessTests(unittest.TestCase):
    ...
    def test_hidden_source_fields_rejected(self):
        document = make_prefix(["CURRENT_COMMITMENT_MISSED"]).to_dict(); document["source_seed"] = "forbidden"
        with self.assertRaises(S0V2Error):
            ObservablePrefixV2.from_dict(document)
    ...
    def test_counted_learner_reads_multiplicity(self):
        ...
        fitted = fit_parameters("B2", [(repeated, "ASSERT_BOUNDARY", "CONFLICT_LOOP")], empty_parameters())
        self.assertNotEqual(predict_full("B2", repeated, fitted)["immediate_action"], predict_full("B2", single, fitted)["immediate_action"])
