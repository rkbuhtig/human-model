"""Non-authoritative candidate routing.

Routing salience and evidential strength never share a type or function.
"""
...
def route_candidates(
    ...
    *,
    ...
    extra_count = min(len(FANOUT_ACTIONS), round(event.candidate_fanout * len(FANOUT_ACTIONS)))
    action_kinds = BASE_ACTIONS + FANOUT_ACTIONS[:extra_count]
    rejected = _rejection_confidence(state)
    ...
    common = {
        "wait": (
            ...
            ("low_pressure", 0.25 * (1.0 - event.time_pressure)),
        ),
        "ask": (
            ("base", 0.15),
        ...
        "accuse": (
            ...
            ("pressure", 0.75 * event.time_pressure),
    ...
    for action in action_kinds:
        terms = tuple(InfluenceTerm(channel=name, delta=value) for name, value in common[action])
        candidates.append(Candidate(candidate_id=f"cand:{event.event_id}:{action}", action_kind=action))
    ...
    return tuple(
        ...
        for candidate, score, probability, terms in zip(candidates, scores, probabilities, terms_by_action)
    )
