...
| Implementation | `INTERP-001A` boundary `DOCUMENTED`; `INTERP-001A2` reception M1 contract `FROZEN`; `INTERP-001B` detached conformance `IMPLEMENTED`; broader TargetForm/Ghost manifest와 runtime integration `UNIMPLEMENTED` |
...
| I-A-07 | 같은 event/material 수·크기에서 access, assembly topology, reception profile과 탐색 경로가 단순 합·threshold와 다른 구조 구분을 만드는지 비교한다. | hypothesis `PROPOSED`; reception access/coherence M1 synthetic conformance `IMPLEMENTED`; TargetForm/Ghost broader manifest `OPEN` |
