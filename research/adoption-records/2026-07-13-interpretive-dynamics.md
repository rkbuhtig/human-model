...
`INTERP-001D1`은 이 hold를 풀지 않는다. D1의 `DeclaredTargetFormReadoutProfile`은
outcome-blind source compiler의 detached projection이고 durable TargetForm object가 아니다.

### I-R-08 — supplied sensitivity와 source derivation을 분리

외생적으로 공급한 TargetForm이 encounter나 candidate를 바꾸는 것과, prior
Episode/Narrative-role source fixture에서 그 readout을 산출하는 것은 서로 다른 주장이다.

```text
SOURCE_COMPILER success
≠ ENCOUNTER_FORMATION sensitivity
≠ GHOST_PATH distinction
```

Block B나 C의 성공을 Block A source claim으로 소급하지 않는다. `Episode`와
