...
이 문서는 PR #31의 head를 기준선으로 삼지만, PR #31의 `work/current-working-model.md`를 소급 검증하거나 승인하지 않는다.
PR #31은 중심 질문면을 임시 연구 설명으로 제안했고, 이 파일럿은 그중 일부와 인접한 Chapter 구분만 시험한다.
따라서 아래 coverage gap은 PR #31의 결론을 수정하는 조용한 우회가 아니라, 후속 작업에서 직접 검증해야 할 미해결 의존성을 표시한다.
