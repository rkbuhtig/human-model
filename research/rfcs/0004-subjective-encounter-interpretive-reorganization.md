...
| 항목 | 값 |
|---|---|
| Status | `PROPOSED — INTERP-001A BOUNDARY DOCUMENTED; INTERP-001A2 M1 MANIFEST FROZEN / UNEXECUTED` |
...
preregistration으로 제시하는 것이다. `INTERP-001A2`는 그중 reception→access와
reception→candidate-coherence 두 간선만 대상으로 exact operator, fixture와 expected
predicate를 동결했다. TargetForm·Ghost·상태 의존적 encounter formation 및 전체
I/TF/EF 비교 manifest는 여전히 후속 blocker다.
...
assembly와 membership identity는 다음 순서로만 만든다. manifest의 stable
`material_key`는 semantic alias일 뿐 runtime `material_ref_id`가 아니다.

```text
MembershipSpec = (material_ref_id, role, order)       # ID 없는 immutable value
assembly_candidate_id
  = digest(
      identity_model_id / version,
      actor_id,
      interpreted_target_scope,
      access_occurrence_id / access_ordinal,
      assembly_operator_declaration_digest,
      ordered MembershipSpec[],
      canonical induced-topology projection
    )
membership_id
  = digest(
      membership_identity_model_id / version,
      assembly_candidate_id,
      MembershipSpec
    )
```

role/order/topology는 candidate 생성 뒤 mutate하지 않는다. material ref는 한 assembly
안에서 중복되지 않고 order는 0부터 연속적이며, embedded spec과 membership record는
정확히 1:1이어야 한다. assembly identity가 membership ID에 의존하지 않으므로 순환이
없고, membership은 이미 식별된 assembly와 material을 잇는다. topology나 operator
version을 빼면 같은 access·member가 다른 조립 규칙 아래 충돌할 수 있으므로 invalid다.
...
- implementation 전 exact operator/fixture/predicate manifest freeze blocker 정의
- 코드와 HumanState 변경 없음

### `INTERP-001A2` — M1 executable manifest freeze

- `R0` state-independent, `R1` access-only, `R2` coherence-only, `R3` factorized
  access+coherence operator 동결
- integer ordinal rank `0/1/2`; equality·ordering·min·max·rank threshold만 허용
- 16개 mirrored fixture × 4개 model의 64-cell matrix와 step별 semantic signature 동결
- execution manifest와 evaluation manifest 분리; runner의 evaluation access 금지
- neutral TargetForm, fixed Ghost, reception-independent encounter formation과 frozen
  fixture topology 사용
- schema와 semantic validator test 구현
- status는 `FROZEN / UNEXECUTED`; receipt runner와 HumanState 변경 없음

M1은 reception이 encounter form 자체를 바꾸는지, TargetForm/Ghost 자유도가 필요한지,
인간 mood·memory 법칙이 맞는지를 시험하지 않는다.
...
7. exact executable manifest는 slice별로 동결하며 M1 freeze를 전체 INTERP manifest
   완료나 실행 결과로 과장하지 않는다.
