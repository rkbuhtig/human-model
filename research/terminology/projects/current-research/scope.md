...
이 배치가 제공하는 것은 **현재 사용 근거**다. 현재 문서가 어떤 구분을 쓰고 있고 어떤 용어 충돌을 해결해야 하는지는 보여주지만, 그 구분이 엔진과 무관한 인간 현상의 독립 개념이라는 **인간 현상 근거**를 제공하지는 않는다.
...
8. 이 배치의 사용 근거만으로 인간 연구 concept에 `KEEP` 판정을 주지 않는다. 엔진과 무관한 인간 현상 근거가 없으면 판정을 보류한다.
