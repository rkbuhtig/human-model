# Chapter 03 Terminology Intake — Basic Term List

이 문서는 Chapter 03의 역사적 명칭과 기호에서 `extraction-map.md`의 후보 항목으로 들어가는 색인이다. 후보의 정의나 설명을 복제하지 않는다.

| 역사적 명칭 | 관찰된 역할 | 추출 항목 | concept ID | 다음 확인 |
|---|---|---|---|---|
| `EventFire` | 변화가 사건으로 점화됐다는 판정 | X-CH03-001 | 미할당 | 외부 occurrence·임계 통과와 경계 |
| `Event` | perc 상태공간 변위 | X-CH03-002 | 미할당 | 실제 발생·장기 변화와 경계 |
| `Event` | 새로움의 직교 잔차 | X-CH03-003 | 미할당 | anomaly·event 판정과 경계 |
| `Event` | marker·segment 경계 후보 | X-CH03-004 | 미할당 | timepoint·절단 권한과 경계 |
| `J` | clock mismatch·탐색 자격 | X-CH03-005 | 미할당 | 관측값 생성과 분리 |
| `Quench` | persistent state 적용 | X-CH03-006 | 미할당 | 인간 채택과 engine application 경계 |
| `Commit` | persistent state 적용 | X-CH03-006 | 미할당 | Quench·adoption과 경계 |
| `CommitEvent` | application 실행·기록 | X-CH03-007 | 미할당 | typed outcome record와 경계 |
| `CommitAnchor` | commit trace 결박 지점 | X-CH03-007 | 미할당 | application gate와 audit 경계 |
| `ULGATE` | commit 전 필수 gate | X-CH03-007 | 미할당 | warrant·governance와 경계 |
| `Ψ` | 가역 후보장 | X-CH03-008 | 미할당 | phenomenal field·workspace와 경계 |
| `View` | 가역 후보·관측 영역 | X-CH03-008 | 미할당 | readout·processing과 경계 |
| `JOT` | 가역 후보장 | X-CH03-008 | 미할당 | process·store와 분리 |
| `JOT` | court·판정 process | X-CH03-009 | 미할당 | 인간 판단·adjudicator와 경계 |
| `JOT` | append-only 비권위 로그 | X-CH03-010 | 미할당 | unresolved store·memory trace와 경계 |
| collapse signature | 실패 구조의 지속 흔적 | X-CH03-019 | 미할당 | learning update·evidence와 경계 |
| `SSOT` | 관측·메모리·에너지 persistent state | X-CH03-011 | 미할당 | 인간 지속 상태·authority ledger와 경계 |
...
| `Π_phys` | 렌즈 독립 에너지·열·갈등 투영 | X-CH03-028 | 미할당 | physical cost·grounding·evidence 경계 |
| `G_phys` | phys grounding basis | X-CH03-028 | 미할당 | external calibration과 경계 |
...
| `A` | Attention | X-CH03-043 | 미할당 | 후속 Accounting·authority `a`와 구분 |
| `W` | Distortion·attenuation | X-CH03-043 | 미할당 | window·witness와 구분 |
| `C^(P)` | 즉시 perc 정렬도 | X-CH03-044 | 미할당 | slow coherence state와 분리 |
| `C_t` | SLOW coherence state | X-CH03-044 | 미할당 | immediate readout과 분리 |
| `g_i` | phys grounding basis | X-CH03-045 | 미할당 | conductivity·gradient와 분리 |
| `g_t` | 전도도 | X-CH03-045 | 미할당 | grounding·qualia gradient와 분리 |
| `g_t` | qualia 방향 gradient | X-CH03-045 | 미할당 | conductivity와 분리 |
| `VOID` | 저활성·저정렬 구역 | X-CH03-015 | 미할당 | `Γ_void`와 동음 재발명 확인 |

## Chapter 02 연결 요약

| Chapter 02 후보 | Chapter 03에서 갈라진 방향 |
|---|---|
| X-CH02-003 외부 발생 | X-CH03-001 사건 점화 / X-CH03-002 perc 변위 / X-CH03-003 새로움 잔차 |
| X-CH02-005·006 단계·시점 | X-CH03-004 marker / X-CH03-005 clock mismatch 탐색 자격 |
| X-CH02-008·009·010 Quench/고정 | X-CH03-006 application / X-CH03-007 gate |
| X-CH02-012·013 JOT process/store | X-CH03-008 candidate field / X-CH03-009 process / X-CH03-010 log |
| X-CH02-015 SSOT state | X-CH03-011 persistent SSOT / X-CH03-012 current scene |
| X-CH02-021·022 Φ surface/routing | X-CH03-013 surface / X-CH03-018 search address / X-CH03-023 influence |
| X-CH02-025 readout/authority | X-CH03-026 proposal-selection split / X-CH03-031~037 authority-flow 규율 |
