# Chapter 02 Terminology Intake — Basic Term List

이 문서는 역사적 명칭에서 `extraction-map.md`의 후보 항목으로 들어가는 색인이다. 후보의 정의나 설명을 복제하지 않는다.
...
| `Potential` | 결론 전 가능성 유지 | X-CH02-011 | 미할당 | 현재 경로분포·candidate 언어와 대조 |
| `Potential` | routing·후보장 | X-CH02-022 | 미할당 | Φ와의 경계 |
| `Σ` | CandidatePack generator | X-CH02-014 | 미할당 | 생성기와 후보 묶음 분리 |
| `Σ` | 생성된 후보 묶음과 혼동 | X-CH02-011 | 미할당 | generator/result 구분 |
| `Λ` | 개인 History·약속·상처·후회 원장 | X-CH02-018 | 미할당 | 자기 역사와 단순 memory store 구분 |
| `Λ` | 신체·지속 흔적 | X-CH02-016 | 미할당 | body trace와 ledger 구분 |
| `Λ` | 관계·책임 회계 | X-CH02-023 | 미할당 | Ledger와 동일성 여부 |
| `Φ` | 현재 phenomenal surface/readout | X-CH02-021 | 미할당 | Chapter 03·11 및 qualia note |
...
| `SSOT` | 비가역 적용 대상 | X-CH02-008 | 미할당 | store와 lock 연산 분리 |
| `SSOT` | 기억 고착 | X-CH02-017 | 미할당 | stored/addressed/accessible 구분 |
| `SSOT` | 현재 표면과 구별되는 writable state | X-CH02-025 | 미할당 | current-research의 readout/state 경계 |

## Chapter 밖에서 제안됐지만 아직 이 색인에 넣지 않은 후보

`실행 허가(authorization)`는 Issue #32와 후속 대화에서 제안됐지만 Chapter 02 중심 synthesis와 Appendix A2에서 독립 역할로 직접 확인되지 않았다. `projects/current-research/`에서 현재 설명상의 필요와 근거를 확인하기 전에는 Chapter 02 후보처럼 등록하지 않는다.
