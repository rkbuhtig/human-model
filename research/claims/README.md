...
`defect_resolution_claims`도 비어 있다. 다음 순서는 P1-v1 사전등록·scripted repilot → ACQ0와
prompt-reactivity gate → source-specific acquisition/human/latent/D2a protocol freeze → 독립 실행 →
cross-source audit(C)다. P0/P1은 actual acquisition이나 claim support를 추가하지 않는다. `HM-INV-013`과
