...
`PROPOSED_NOT_ADOPTED / UNEXECUTED` revision proposal을 발행했다. P0-v1은 이를 exact
candidate/decision lineage와 closed-world surface로 채택·동결했지만 아직 실행하지 않았고,
`defect_resolution_claims`도 비어 있다. immediate mainline은 claim-support를 바꾸지 않는
`INTERP-001D2a0` contract freeze → detached D2a1 → `OBS-MAP-000`으로 전환됐다. P1-v1은
non-mainline hold이며 defect resolution 전에 여전히 별도 preregistration/replay가 필요하다.
그 사이 `INTERP-001D2a0-EXEC0`는 exact operator/lifecycle, execution unit과 evaluator selector를
동결할 뿐 runner, evaluator implementation 또는 result를 추가하지 않는다. P0/P1/D2a0/EXEC0
뒤의 `INTERP-001D2a0-MAT0`도 materialization/publication ABI만 동결하며 실행하지 않는다.
P0/P1/D2a0/EXEC0/MAT0 contract validation은 actual acquisition이나 claim support를 추가하지 않는다. `HM-INV-013`과
